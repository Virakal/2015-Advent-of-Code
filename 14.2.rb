def calc time, input
  ...
  time.times do |s|
    ...
    leader_distance = deer.max_by { |d| d[:travelled] }[:travelled]
    leaders = deer.select { |d| d[:travelled] == leader_distance }
    leaders.each { |d| d[:points] += 1 }
  end

  return (deer.max_by { |d| d[:points] })[:points]
end
