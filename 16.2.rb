def find sues, to_find
  sues.lines.each do |line|
    sue = parse_sue line
    found = true
    ...
    1.upto 3 do |i|
      ...
      q = sue["q#{i}"].to_i
      op = get_op t
      compare = to_find[t.to_sym]

      if !q.send op, compare
        found = false
        break
      end
    end

    break sue['id'] if found
  end
end

def parse_sue line
  @re ||= /^Sue (?<id>\d+): (?<t1>.+): (?<q1>\d+), (?<t2>.+): (?<q2>\d+), (?<t3>.+): (?<q3>\d+)$/
  line.match @re
end

def get_op t
  return :> if ["cats", "trees"].include? t
  return :< if ["pomeranians", "goldfish"].include? t
  return :==
end
