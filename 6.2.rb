def process instructions
  grid = Array.new(1000) { |arr, x| Array.new(1000) { |arr2, x2| 0 } }
  re = /^(?<command>turn o(?:n|ff)|toggle) (?<x1>\d+),(?<y1>\d+) through (?<x2>\d+),(?<y2>\d+)$/

  instructions.lines.each do |line|
    follow_instruction grid, line.match(re)
  end

  count_lit_lights grid
end

def follow_instruction grid, ins
  if ins[:command] == 'turn on'
    cb = Proc.new {|x| x + 1 }
  elsif ins[:command] == 'turn off'
    cb = Proc.new {|x| [x - 1, 0].max }
  elsif ins[:command] == 'toggle'
    cb = Proc.new {|x| x + 2 }
  else
    raise 'Weird instruction ' + ins[:command].to_s
  end

  (ins[:x1].to_i..ins[:x2].to_i).each do |x|
    (ins[:y1].to_i..ins[:y2].to_i).each do |y|
      grid[x][y] = cb.call grid[x][y]
    end
  end
end
